/// @brief Test that we can write a Zarr V3 dataset where the chunk size exceeds
/// the array size.
...
const static uint32_t chunk_width = frame_width + 1; // exceeds frame_width
const static uint32_t shard_width = 1;

const static uint32_t frame_height = 48;
const static uint32_t chunk_height = frame_height + 1; // exceeds frame_height
const static uint32_t shard_height = 8;

const static uint32_t max_frame_count = 16;
const static uint32_t frames_per_chunk = 17; // exceeds max_frame_count
...
validate()
{
    ...
    for (auto t = 0; t < std::ceil(max_frame_count / frames_per_chunk); ++t) {
        fs::path path = test_path / "0" / "c" / std::to_string(t) / "0" / "0";
