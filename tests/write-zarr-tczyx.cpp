...
const static uint32_t chunk_height = 320;
const static uint32_t chunk_slice = 8;
...
acquire(AcquireRuntime* runtime, const char* filename)
{
    ...
    props.video[0].max_frame_count = num_time_points * num_channels * num_slices;
...
main()
{
    ...
    ASSERT_EQ(int, "%d", num_time_points, shape[0]);
    ASSERT_EQ(int, "%d", num_channels, shape[1]);
    ASSERT_EQ(int, "%d", num_slices, shape[2]);
    ASSERT_EQ(int, "%d", frame_height, shape[3]);
    ASSERT_EQ(int, "%d", frame_width, shape[4]);
    LOG("shape OK");

    auto chunks = zarray["chunks"];
    ASSERT_EQ(int, "%d", 1, chunks[0]);
    ASSERT_EQ(int, "%d", 1, chunks[1]);
    ASSERT_EQ(int, "%d", chunk_slice, chunks[2]);
    ASSERT_EQ(int, "%d", chunk_height, chunks[3]);
    ASSERT_EQ(int, "%d", chunk_width, chunks[4]);
    LOG("chunks OK");
