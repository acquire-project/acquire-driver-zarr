...
setup()
{
    ...
    dimension = DIM("t", ZarrDimensionType_Time, array_timepoints, chunk_timepoints, shard_timepoints);
    ZarrStreamSettings_set_dimension(settings, 0, &dimension);

    dimension = DIM("c", ZarrDimensionType_Channel, array_channels, chunk_channels, shard_channels);
    ZarrStreamSettings_set_dimension(settings, 1, &dimension);

    dimension = DIM("z", ZarrDimensionType_Space, array_planes, chunk_planes, shard_planes);
    ZarrStreamSettings_set_dimension(settings, 2, &dimension);

    dimension = DIM("y", ZarrDimensionType_Space, array_height, chunk_height, shard_height);
    ZarrStreamSettings_set_dimension(settings, 3, &dimension);

    dimension = DIM("x", ZarrDimensionType_Space, array_width, chunk_width, shard_width);
