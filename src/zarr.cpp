...
size_t
zarr::get_bytes_per_frame_(const ImageShape& image_shape) noexcept
{
...
{
    try {
        return new zarr::Zarr<zarr::Buffer<zarr::RawFile>>();
    } catch (const std::exception& exc) {
        LOGE("Exception: %s\n", exc.what());
    } catch (...) {
        LOGE("Exception: (unknown)");
