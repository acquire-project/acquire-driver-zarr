...
#define BLOSC_LZ4HC_FORMAT                                                     \
    BLOSC_LZ4_LIB /* LZ4HC and LZ4 share the same format */
...
/* Split mode for blocks.  NEVER and ALWAYS are for experimenting with best
 * compression ratio, AUTO for optimal behaviour (based on experiments), and
 * FORWARD_COMPAT provides best forward compatibility */
...
#define BLOSC_FORWARD_COMPAT_SPLIT 4
    ...
    /**
      ...
      */
    BLOSC_EXPORT int blosc_compress(int clevel,
                                    int doshuffle,
                                    size_t typesize,
                                    size_t nbytes,
                                    const void* src,
    ...
    */
    ...
    /**
      ...
      Call `blosc_cbuffer_validate` to determine the size of the destination
      buffer.
    ...
    */
    BLOSC_EXPORT int blosc_decompress(const void* src,
                                      void* dest,
                                      size_t destsize);
    ...
    /**
      ...
      will be returned instead.
    */
    BLOSC_EXPORT int blosc_decompress_ctx(const void* src,
                                          void* dest,
                                          size_t destsize,
                                          int numinternalthreads);
    ...
    /**
      ...
      some error happens.
      */
    BLOSC_EXPORT int blosc_getitem(const void* src,
                                   int start,
                                   int nitems,
                                   void* dest);
    ...
    /**
      ...
      returned.
     */
    BLOSC_EXPORT int blosc_compcode_to_compname(int compcode,
                                                const char** compname);
    ...
    */
    ...
    /**
      ...
      */
    BLOSC_EXPORT int blosc_get_complib_info(const char* compname,
                                            char** complib,
                                            char** version);
    ...
    /**
      ...
      If the format is not supported by the library, all output arguments will
      be filled with zeros.
      */
    BLOSC_EXPORT void blosc_cbuffer_sizes(const void* cbuffer,
                                          size_t* nbytes,
                                          size_t* cbytes,
                                          size_t* blocksize);

    /**
      Checks that the compressed buffer starting at `cbuffer` of length `cbytes`
      may contain valid blosc compressed data, and that it is safe to call
      blosc_decompress/blosc_decompress_ctx/blosc_getitem.

      On success, returns 0 and sets *nbytes to the size of the uncompressed
      data. This does not guarantee that the decompression function won't return
      an error, but does guarantee that it is safe to attempt decompression.

      On failure, returns -1.
     */
    BLOSC_EXPORT int blosc_cbuffer_validate(const void* cbuffer,
                                            size_t cbytes,
                                            size_t* nbytes);
    ...
    /**
      ...
      be filled with zeros.
      */
    BLOSC_EXPORT void blosc_cbuffer_metainfo(const void* cbuffer,
                                             size_t* typesize,
                                             int* flags);
    ...
    /**
      ...
      This function should always succeed.
      */
    BLOSC_EXPORT void blosc_cbuffer_versions(const void* cbuffer,
                                             int* version,
                                             int* compversion);
