...
Scaler::Scaler(const ImageShape& image_shape,
...
void
Scaler::push_frame(TiledFrame* frame)
{
    CHECK(frame);
...
bool
Scaler::has_frame(uint64_t frame_id) const
{
...
const TiledFrame*
Scaler::pop_frame_and_make_current()
{
    if (frame_ptrs_.empty()) {
...
const ImageShape&
Scaler::image_shape() const noexcept
{
    return image_shape_;
}

const TileShape&
Scaler::tile_shape() const noexcept
{
    return tile_shape_;
}

int16_t
Scaler::max_layer() const noexcept
{
    return max_layer_;
}

uint8_t
Scaler::downscale() const noexcept
{
    return downscale_;
}

std::mutex&
Scaler::mutex() noexcept
{
    return mutex_;
}
